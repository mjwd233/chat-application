...
useEffect(() => {
  ...
  const checkUnreadMessages = () => {
    const unreadMessages = document.querySelectorAll(
      `.${messageClass}.${styles.notowner}:not([data-seen])`
    );

    if (isWindowFocused && unreadMessages.length > 0) {
  ...
  const messageElements = document.querySelectorAll(`.${messageClass}.${styles.notowner}`);
  messageElements.forEach(el => visibilityObserver.observe(el));

  // Listen for status updates from receiver
  ...
  const renderFile = (fileURL) => {
    ...
    const handleDownload = () => {
      ...
      console.log('Request URL:', `http://localhost:5000/uploads/${filename}`);
      xhr.onload = function() {
        if (this.status === 200) {
    ...
    // Your existing render conditions remain the same
